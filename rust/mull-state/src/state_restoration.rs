...
fn load_raw_report(database_path: &str) -> SqliteResult<RawReport> {
    ...
    let mut stmt = conn.prepare(
        "SELECT mutant_id, execution_status, exit_status, duration, stdout, stderr FROM mutant",
    )?;
    let rows = stmt.query_map([], |row| {
        ...
        Ok((mutant_id, execution_result))
    })?;

    for row in rows {
        let (identifier, execution_result) = row?;
        execution_results
            .entry(Mutant { identifier })
            .or_default()
...
pub fn restore_execution_state(diag: &MullDiagnostics, database_path: &str) -> ExecutionState {
    ...
    match load_raw_report(database_path) {
        Ok(report) => {
