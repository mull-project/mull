...
// RUN: cd %CURRENT_DIR
// RUN: unset TERM; %MULL_EXEC -test-framework CustomTest -mutators=cxx_div_to_mul -ide-reporter-show-killed -reporters=IDE %s.exe | %FILECHECK_EXEC %s --dump-input=fail
