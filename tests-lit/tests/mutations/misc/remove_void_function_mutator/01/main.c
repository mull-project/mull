...
// RUN: cd %CURRENT_DIR
// RUN: unset TERM; %MULL_EXEC -test-framework CustomTest -mutators=remove_void_function_mutator -ide-reporter-show-killed -reporters=IDE %s.exe | %FILECHECK_EXEC %s --dump-input=fail
