...
// RUN: cd %CURRENT_DIR
// RUN: unset TERM; %MULL_EXEC -test-framework CustomTest -mutators=cxx_logical_or_to_and -mutators=cxx_logical_and_to_or -ide-reporter-show-killed -reporters=IDE %s.exe | %FILECHECK_EXEC %s --dump-input=fail
