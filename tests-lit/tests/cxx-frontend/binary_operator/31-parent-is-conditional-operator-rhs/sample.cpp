...
RUN: (env "cxx_logical_or_to_and:%s:6:28"=1 %s.exe || true) | %FILECHECK_EXEC %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=STANDALONE_WITH_MUTATION
...
MULL_RUNNER:{{.*}}sample.cpp:6:28: warning: Killed: Replaced || with && [cxx_logical_or_to_and]
*/
