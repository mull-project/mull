...
RUN: MULL="cxx_add_to_sub:%s:6:12:1" %s.exe | %FILECHECK_EXEC %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=RIGHT_MUTATION_WITH
...
RUN: MULL="cxx_add_to_sub:%s:6:12:1" %s.exe | %FILECHECK_EXEC %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=WRONG_MUTATION_WITH
