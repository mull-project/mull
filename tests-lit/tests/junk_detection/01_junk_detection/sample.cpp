...
RUN: cd %CURRENT_DIR
RUN: %MULL_EXEC -workers=1 -test-framework CustomTest -disable-junk-detection -mutators=cxx_add_to_sub -mutators=remove_void_function_mutator -reporters=IDE %s.exe | %FILECHECK_EXEC %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=WITHOUT-JUNK-DETECTION
RUN: %MULL_EXEC -workers=1 -test-framework CustomTest -mutators=cxx_add_to_sub -mutators=remove_void_function_mutator -reporters=IDE -ide-reporter-show-killed -compdb-path %S/compile_commands.json %s.exe | %FILECHECK_EXEC %s --dump-input=fail --strict-whitespace --match-full-lines --check-prefix=WITH-JUNK-DETECTION
