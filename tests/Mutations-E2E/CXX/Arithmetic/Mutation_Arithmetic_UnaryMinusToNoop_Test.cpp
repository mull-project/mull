...
TEST(Mutation_Arithmetic_UnaryMinusToNoop, End_2_End) {
  ...
  ASSERT_EQ(singleFileMutations.count(MutatorKind::CXX_UnaryMinusToNoop), 1U);

  ASSERT_EQ(singleFileMutations[MutatorKind::CXX_UnaryMinusToNoop].size(), 1U);
  ASSERT_EQ(singleFileMutations[MutatorKind::CXX_UnaryMinusToNoop].count(locationHash), 1U);
