...
// RUN: %mull_runner %s-ir.exe --allow-surviving -ide-reporter-show-killed -test-program %python3 -- test.py %s-ir.exe "first test case" | %filecheck %s --dump-input=fail --match-full-lines --check-prefix=CHECK-TEST1
...
// RUN: %mull_runner %s-ir.exe --allow-surviving -ide-reporter-show-killed -test-program %python3 -- test.py %s-ir.exe "second test case" | %filecheck %s --dump-input=fail --match-full-lines --check-prefix=CHECK-TEST2
