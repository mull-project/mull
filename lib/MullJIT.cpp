...
void MullJIT::addLogicalModule(llvm::Module *module) {
  ...
  std::list<LogicalModule>::iterator logicalModuleIterator = std::prev(logicalModules.end());
  LogicalModuleResources &resources = logicalModuleIterator->Resources;
  resources.SourceModule = module;
  ...
  const DataLayout &DL = module->getDataLayout();
  {
    ...
    for (auto &F : *module) {
      ...
      resources.StubsToClone.insert(&F);
      ...
      CCInfo.setCompileAction([this, logicalModuleIterator, &F, index]() {
        return this->extractAndCompile(logicalModuleIterator, F, index);
      });
    }

    resources.StubsMgr = stubsManagerBuilder();
//      LMResources.StubsMgr = make_unique<orc::LocalIndirectStubsManager<orc::OrcX86_64_SysV>>();
    auto EC = resources.StubsMgr->createStubs(StubInits);
    (void) EC;
  ...
  auto Materializer = createLambdaMaterializer(
      [this, &GVsM, &resources](Value *V) -> Value * {
        if (auto *F = dyn_cast<Function>(V)) {
          ...
          auto StubSym = resources.StubsMgr->findStub(FName, false);
  ...
  auto GVsResolver = orc::createLambdaResolver(
      [this, logicalModuleIterator](const std::string &Name) {
...
llvm::orc::TargetAddress MullJIT::extractAndCompile(std::list<LogicalModule>::iterator LMH,
                                                    Function &F,
  ...
  if (auto EC = resources.StubsMgr->updatePointer(FnName, FnBodyAddr))
    return 0;
...
MullJIT::BaseLayerModuleSetHandleT MullJIT::emitFunction(std::list<LogicalModule>::iterator LMH,
  ...
  auto Materializer = createLambdaMaterializer([this, &resources, &M,
    ...
    if (auto *F = dyn_cast<Function>(V)) {
      ...
      if (!resources.StubsToClone.count(F))
